...
<template lang="">
  <div class="card">
    <img :src="generateCoverImage(serie.poster_path)" class="cover" alt="" />
    <h1>{{ serie.name }}</h1>
    <h2>{{ serie.original_name }}</h2>
    <div>{{ vote(serie.vote_average) }}</div>
    <img :src="generateFlagImageUrl(serie.original_language)" alt="" />
    ...
    <div v-for="starIndex in emptyStars(serie.vote_average)" :key="starIndex">
      <svg
