...
<template lang="">
  <div class="card">
    <img :src="generateCoverImage(film.poster_path)" alt="" class="cover" />

    <div class="info">
      <h1>{{ film.title }}</h1>
      <h2>{{ film.original_title }}</h2>
      <img
        :src="generateFlagImageUrl(film.original_language)"
        class="language_flag"
        alt=""
      />
      ...
      <div class="stars">
        <div v-for="starIndex in vote(film.vote_average)" :key="starIndex">
          <svg
        ...
        <div
          v-for="starIndex in emptyStars(film.vote_average)"
          :key="starIndex"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
